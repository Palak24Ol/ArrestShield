...
| Simple professor-demo detector | Word/character TF-IDF plus SGD over all cleaned relevant sources and mixed-label hard examples | `major_project`, `reports/simple_content_detector_v1` | Packaged; 18/20 behavioral cases, including 10/10 scams |
...
| Artifact integrity | Direct data/model hashes and policy-boundary checks | `reports/verification_v1/verification.json` | Passed 37/37 with no skips |
...
4. The full automated suite passes (101 repository tests plus 2 packaged-project tests).
5. `scripts/verify_local_artifacts.py` passes with a completed multi-task artifact required (37/37 checks).
